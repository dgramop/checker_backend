...
use rocket::{serde::json::Json, form::Form, State};
use tl::{ParserOptions, NodeHandle, Node};
pub mod schema;
...
async fn get_client() -> Result<Client, reqwest::Error> {
    let reqw = reqwest::ClientBuilder::new().cookie_store(true).build().expect("builds reqwest client");

    //TODO: Clean up
    let login_body = reqw.post("https://awrgmu.atriumcampus.com/activity/mix/do_login")
        .form(&[("username",env::var("ATRIUM_USERNAME").expect("ATRIUM_USERNAME env variable must be set")),("password", env::var("ATRIUM_PASSWORD").expect("ATRIUM_PASSWORD env variable must be set"))])
        .send()
        .await?
        .text()
        .await?;
...
async fn take_workshop(gnum: u32, workshop: uuid::Uuid) -> Result<Json<(Taken, Workshop)>, Json<TakeWorkshopError>> {
    let mut conn = establish_connection();

    let _member = match members::dsl::members.filter(members::dsl::gnum.eq(gnum as i32)).get_result::<Member>(&mut conn) {
        Ok(member) => member,
        _ => {
    ...
    let workshop = match workshops::dsl::workshops.filter(workshops::dsl::id.eq(workshop.to_string())).get_result::<Workshop>(&mut conn) {
    ...
    match diesel::insert_into(taken::dsl::taken).values(Taken {
    ...
    match taken::dsl::taken.filter(taken::dsl::id.eq(inserted_taken_id)).get_result::<Taken>(&mut conn) {
...
async fn untake_workshop(gnum: u32, workshop: uuid::Uuid) -> Result<Json<(Taken, Workshop)>, Json<TakeWorkshopError>> {
    let mut conn = establish_connection();
    ...
    let to_delete = taken::dsl::taken.filter(taken::dsl::member.eq(gnum as i32)).filter(taken::dsl::workshop.eq(&workshop.id)).clone();
...
async fn delete_workshop(workshop: uuid::Uuid) -> Result<(), ()> {
    ...
    match diesel::delete(workshops::dsl::workshops.filter(workshops::dsl::id.eq(workshop.to_string()))).execute(&mut conn) {
        Ok(_) => {
            Ok(())
        },
        Err(e) => {
...
async fn add_workshop(workshop_form: Form<CreateWorkshopForm>) -> Result<(), ()> {
    let mut conn = establish_connection();
    match diesel::insert_into(workshops::dsl::workshops).values(Workshop {
...
async fn check_in(id: String, state: &State<St>) -> Json<CheckInResp> {//Json<CheckInResponse> {

    //let reqw = get_client().await.unwrap();

    let atrium_req = state.client.read().await.client.post("https://awrgmu.atriumcampus.com/activity/mix/ajax/basic_search")
        .form(&[("card_number",id)]);
        
    let mut atrium = atrium_req.try_clone().expect("atrium request should be cloneable").send()
        .await.unwrap()
        .json::<CheckInAtrium>()
        .await.unwrap();
    ...
    // request
    match &atrium {
        CheckInAtrium::Undetailed { success, message } if *success == false && message == "log_out" => {
            state.client.write().await.client =  get_client().await.expect("should log-in to atrium");
            eprintln!("Was logged out of atrium, logging in and trying again");
            atrium = atrium_req.send()
                .await.unwrap()
                .json::<CheckInAtrium>()
                .await.unwrap();
        },
    ...
    match atrium {
        CheckInAtrium::Detailed {success: _, html, eligibility } =>  {
            ...
            let (name, gnum, music):(_, _, Option<String>) = {
                ...
                let person_name: String = match parsed_html.get_element_by_id("person_name") {
                    Some(name) => {
                        name.get(parser).unwrap().inner_text(parser).to_string()
                    },
                    None => {
                        panic!()
                    }
                ...
                let gnum: u32 = match parsed_html.get_elements_by_class_name("campus_id").next() {
                    Some(id) => {
                        ...
                        for child in NodeHandle::get(&id, parser).unwrap().children().unwrap().top().iter() {
                            if let Node::Raw(b) = NodeHandle::get(child, parser).unwrap() {
                                let candidate_id = b.as_utf8_str().to_string();
                                eprintln!("{}",candidate_id);

                                if let Ok(parsed_integer_id) = u32::from_str_radix(&candidate_id.trim(), 10) {
            ...
            let workshops: Vec<(Taken, Workshop)> = match taken::dsl::taken.filter(taken::dsl::member.eq(gnum as i32)).inner_join(workshops::dsl::workshops.on(workshops::dsl::id.eq(taken::dsl::workshop))).get_results::<(Taken, Workshop)>(&mut conn) {
                Ok(workshops) => workshops,
                Err(e) => {
                    eprintln!("Non-fatal error loading workshops for gnum {} with error: {:?}", gnum, e);
                    Vec::new()
                }
            };
        ...
        CheckInAtrium::Undetailed {success: _, message} => {
...
#[launch]
async fn rocket() -> _ {
    rocket::build().manage(St {
        client: RwLock::new(AtriumClient { client: get_client().await.expect("client logs in") })
    }).mount("/", routes![check_in, take_workshop, list_workshops, add_workshop, untake_workshop, delete_workshop])
}

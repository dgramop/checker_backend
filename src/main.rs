...
async fn get_client(secrets: &Secrets) -> Result<Client, reqwest::Error> {
    ...
    let login_body = reqw
        ...
        .form(&[
            ("username", &secrets.atrium_username),
            ("password", &secrets.atrium_password),
        ])
        .send()
        .await?
        .text()
...
async fn check_in(id: String, state: &State<St>) -> Json<CheckInResp> {
    ...
    match atrium {
        CheckInAtrium::Detailed {
            ...
            let (name, gnum, _music): (_, _, Option<String>) = {
            ...
            let music: Option<String> = match members::dsl::members
                .filter(members::dsl::gnum.eq(gnum as i32))
                .get_result::<Member>(&mut conn)
            {
...
fn get_secrets() -> Secrets {
    ...
    secrets.expect(&format!(
        "Unable to deserialize secrets. See schema template:\n{template}"
    ))
}
    ...
    rocket::build()
        .manage(St {
            client: RwLock::new(AtriumClient {
                client: get_client(&secrets).await.expect("client logs in"),
            }),
            secrets,
        })
        .mount(
